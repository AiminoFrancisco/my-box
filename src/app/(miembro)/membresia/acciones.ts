...
export async function subirComprobante(
  ...
  if (!archivo || archivo.size === 0) return { error: "Selecciona el archivo del comprobante." };
  if (archivo.size > 10 * 1024 * 1024) return { error: "El archivo es muy grande (máx. 10 MB)." };
  ...
  if (errUp) return { error: "No se pudo subir el archivo. Intenta de nuevo." };
  ...
  if (errIns) return { error: "No se pudo registrar el comprobante." };
