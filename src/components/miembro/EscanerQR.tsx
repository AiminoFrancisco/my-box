...
export function EscanerQR({ activo }: { activo: boolean }) {
  ...
  async function alDetectar(texto: string) {
    ...
    if (r.herramienta) {
      ...
      setErrorBusqueda(r.error ?? dic.member.escaner.errorBusquedaFallback);
      setFase("inicio");
    }
  }
  ...
  async function iniciarCamara() {
    ...
    try {
      ...
      setFase("inicio");
      setErrorBusqueda(dic.member.escaner.errorCamara);
    }
  }
  ...
  if (!activo) {
    return (
      <div className="rounded-3xl border border-alerta/30 bg-alerta/5 p-8 text-center">
        ...
        <p className="mt-3 text-lg font-semibold text-marca-marino">{dic.member.escaner.inactivaTitulo}</p>
        <p className="mt-1 text-sm text-tenue">{dic.member.escaner.inactivaTexto}</p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center">
        ...
        <h2 className="mt-4 font-display text-xl font-extrabold text-marca-marino">
          {resultado.ok ? dic.member.escaner.resultadoOk : dic.member.escaner.resultadoError}
        </h2>
        ...
        >
          <RefreshCw className="h-5 w-5" /> {dic.member.escaner.escanearOtra}
        </button>
      </div>
    );
  }
      ...
      <div className="space-y-5">
        <div className="overflow-hidden rounded-3xl border border-borde bg-superficie shadow-glow">
          <div className="relative aspect-video bg-fondo">
            ...
            <div className="absolute left-3 top-3">
              <BadgeEstado color={meta.color as "exito" | "alerta" | "peligro"}>{dic.common.estados.herramienta[herr.estado]}</BadgeEstado>
            </div>
          </div>
          <div className="p-5">
            <span className="text-xs font-semibold uppercase tracking-wide text-marca-azul">{herr.categoria ?? dic.member.escaner.categoriaFallback}</span>
            ...
            <p className="mt-3 text-sm text-tenue">{dic.member.escaner.valorReemplazo} <strong className="text-marca-marino">{formatoDinero(herr.valor_reemplazo)}</strong></p>
          </div>
        </div>

        <div className="rounded-2xl bg-marca-azul/5 p-4 text-center text-sm text-marca-marino">
          {modo === "sacar"
            ? dic.member.escaner.confirmarSacar
            : dic.member.escaner.confirmarDevolver}
        </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          >
            ...
            {modo === "sacar" ? dic.member.escaner.siSacar : dic.member.escaner.siDevolver} <ArrowRight className="h-5 w-5" />
          </button>
          <button onClick={reiniciar} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-borde px-6 py-3 text-sm font-semibold text-tenue active:scale-[0.98]">
            <X className="h-4 w-4" /> {dic.member.escaner.cancelar}
          </button>
        </div>
      </div>
    );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-2xl bg-fondo p-1.5">
        {(["sacar", "devolver"] as Modo[]).map((m) => (
          ...
          >
            ...
            {m === "sacar" ? dic.member.escaner.modoSacar : dic.member.escaner.modoDevolver}
          </button>
        ))}
      </div>
        ...
        {fase !== "escaneando" && (
          <div className="flex flex-col items-center gap-3 px-6 py-12 text-center">
            ...
            <p className="text-sm text-tenue">
              {fase === "buscando" ? dic.member.escaner.buscando : dic.member.escaner.apunta}
            </p>
            <button
            ...
            >
              <Camera className="h-5 w-5" /> {dic.member.escaner.abrirCamara}
            </button>
          </div>
        )}
      </div>
      ...
      {fase === "escaneando" && (
        <button onClick={() => { detener(); setFase("inicio"); }} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-borde px-5 py-3 text-sm font-semibold text-tenue active:scale-[0.98]">
          <CameraOff className="h-4 w-4" /> {dic.member.escaner.detenerCamara}
        </button>
      )}
      ...
      >
        <label className="flex items-center gap-2 text-sm font-medium text-marca-marino">
          <Keyboard className="h-4 w-4 text-marca-azul" /> {dic.member.escaner.manualLabel}
        </label>
        <div className="mt-2 flex gap-2">
          <input
            value={manual}
            onChange={(e) => setManual(e.target.value)}
            placeholder={dic.member.escaner.manualPlaceholder}
