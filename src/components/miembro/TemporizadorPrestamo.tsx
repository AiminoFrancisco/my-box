...
export function TemporizadorPrestamo({
  ...
  if (devuelto) {
    return (
      <span className="inline-flex items-center gap-1.5 rounded-full bg-exito/10 px-3 py-1 text-xs font-medium text-exito">
        <CheckCircle2 className="h-3.5 w-3.5" /> Devuelta
      </span>
    );
  }
  ...
  const estilos = {
    verde: { barra: "bg-exito", chip: "bg-exito/10 text-exito", Icono: Clock, texto: `Quedan ${dias}d ${horas}h` },
    por_vencer: { barra: "bg-alerta", chip: "bg-alerta/10 text-amber-700", Icono: AlertTriangle, texto: `¡Por vencer! ${horas}h` },
    vencido: { barra: "bg-peligro", chip: "bg-peligro/10 text-peligro", Icono: AlertTriangle, texto: `Vencido hace ${dias}d ${horas}h` },
  }[estado];
